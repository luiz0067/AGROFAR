...
<script language="JavaScript" >
	function Enviar(){
		document.frmSendmail.body.value = frames.message.document.body.innerHTML;
		$('ver_html').checked=false;
	}
	function $(id){
		//retorna o obejto conforme o id
...
If request("ac") = "editar" Then
	...
	Sqltext = "SELECT * FROM menu_pgs WHERE id_menu_it = " & request("id_menu_it") & ""
...
<FORM METHOD="post" ACTION="menu_pg_it_cadastrar.asp?ac=<%=request("ac")%>&id_menu=<%=request("id_menu")%>" onSubmit="return CheckForm();" name="frmSendmail">

<input name="data" type="hidden" value="<%=data%>">
<input name="id_menu_it" type="hidden" value="<%=Request("id_menu_it")%>">
...
%>
                ...
                <tr> 
                  <td align="right">
                  	<font color="<%=cor_links%>">
                    	<input type="checkbox" onClick="setMode(this.checked)" class='checkbox' id="ver_html">
                        <strong>
			 ...
			 %>
			  ...
			  OnClick="Enviar()"   style="font-family: Arial; font-size: 8 pt; color: <%=cor_links%>">
                    </font></td>
                </tr>
              </table>
              <!-- Fim da Tabela de Fundo -->

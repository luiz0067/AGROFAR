...
<!--#include file=".\config_site.inc"-->
  <%
if request("ac") = "atualizar" Then
  Sql="UPDATE  configuracoes_site SET contato_txt = '" & Request.form("body") &  "' WHERE id_geral =1"
 Conn.Execute(Sql)
 response.Redirect("contato_default.asp")
End If
set Rs = server.createobject("ADODB.recordset")
		Sqltext = "SELECT * FROM configuracoes_site"
		Set Rs = Conn.Execute(Sqltext)
		
	Session("texto") = Rs("contato_txt")
	
	Rs.Close
	Set Rs = Nothing

	%>
  <FORM METHOD="post" ACTION="?ac=atualizar" onSubmit="return CheckForm();" name="frmSendmail">
        ...
        <table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF"  >
          <tr> 
            <td background="../imagens/forum_images/table_bg2.gif"><div align="center"><font color="#336699" size="2" face="Arial, Helvetica, sans-serif"><strong>Editor 
                Interativo para cadastro da P&aacute;gina do Formul&aacute;rio</strong></font></div></td>
          </tr>
          <tr> 
...
%>
		    	...
		    	document.write ('<iframe src="news_textobox.asp?ID=<% = CInt(RND * 2000) %><% Response.Write("&mode=show") %>" id="message" width="100%" height="300"></iframe>')

...
msg = replace(msg,"'","''") 
Sql="INSERT INTO chamada (chamada) VALUES('" & msg & "')"
   Conn.Execute(Sql)  
 
		   
response.Redirect("chamada_default_admin.asp?msg=1")
End If


	%>
  <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong> </strong></font> </div>
<FORM METHOD="post" ACTION="chamada_cadastrar_admin.asp?editar=ok" onSubmit="return CheckForm();" name="frmSendmail">
...
%>
		    	...
		    	document.write ('<iframe src="news_textobox.asp?ID=<% = CInt(RND * 2000) %>" id="message" width="90%" height="150"></iframe>')
